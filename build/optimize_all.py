...
Operations:
  A. Inject GA4 loader (<script async src="/js/analytics.js?v=12.19">) before </head> on all HTML.
  B. Bump main.js references to ?v=12.19 so SW + lang toggle load fresh.
...
GA_SNIPPET = '  <script async src="/js/analytics.js?v=12.19"></script>\n'
...
def edit_file(path, changes):
    ...
    # A. GA4 injection
    if 'analytics.js?v=12.19' not in t:
        t = t.replace('</head>', GA_SNIPPET + '</head>', 1)
        changes.append('GA4 snippet')

    # B. main.js version bump
    new_t = t
    new_t = new_t.replace('src="js/main.js"', 'src="js/main.js?v=12.19"')
    new_t = new_t.replace('src="../js/main.js"', 'src="../js/main.js?v=12.19"')
    if new_t != t:
        t = new_t
        changes.append('main.js?v=12.19')
